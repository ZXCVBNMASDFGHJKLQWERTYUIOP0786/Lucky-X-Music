...
async def chat_gpt(app, message):
    try:
        ...
        if len(message.command) < 2:
            ...
            await message.reply_text("**Hello sir, I am Raja. How can I help you today?**")
        else:
...
async def chat_arvis(app, message):
    try:
        ...
        if len(message.command) < 2:
            await message.reply_text(f"**Hello {name}, I am Raja. How can I help you today?**")
        else:
            query = message.text.split(' ', 1)[1]
            MODEL = "gpt-3.5-turbo"
            resp = openai.ChatCompletion.create(model=MODEL, messages=[{"role": "user", "content": query}],
...
# Command for Luckywith user's name
